...
select c.customer_id , c.customer_name , count(distinct p.product_category ) as counts
from Customers as c  left join purchases as p 
on c.customer_id = p.customer_id  
group by c.customer_id , c.customer_name 
having count(distinct p.product_category ) = (select count(distinct product_category ) from purchases );

---- for distinct count of product_category 
select count(distinct product_category ) from purchases ;

select count( product_category ) from purchases  group by customer_id ; 